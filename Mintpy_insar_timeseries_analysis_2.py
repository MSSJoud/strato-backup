...
work_dir = Path.home() / 'data_aoi_bologna'
data_dir = work_dir  # Since your data is directly in ~/data_aoi_bologna
...
# Unzip the interferograms only if zipped files exist
zip_files = list(work_dir.glob('*.zip'))
if zip_files:
    print("Found zipped files. Extracting...")
    for zip_file in zip_files:
        with zipfile.ZipFile(zip_file, 'r') as zip_ref:
            zip_ref.extractall(data_dir)
else:
    print("No zipped files found. Assuming data is already unzipped.")
...
print(f"Common overlap coordinates: {overlap}")
...
output_dir = work_dir / 'time_series_output'
...
subprocess.run(['smallbaselineApp.py', '--dir', str(output_dir), str(mintpy_config)])
...
# view.main([f'{work_dir}/velocity.h5'])
tsview.main([f'{output_dir}/timeseries.h5'])
view.main([f'{output_dir}/velocity.h5'])

import h5py

# Load the HDF5 file
with h5py.File(f'{output_dir}/timeseries.h5','r') as f:
