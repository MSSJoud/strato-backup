...
class ContextConditionedTwoLayerSwinUNet3D(nn.Module):
    """Two-layer Swin U-Net conditioned on deformation plus static context maps.

    Channel 0 is assumed to be the deformation history. Any remaining channels are
    treated as auxiliary context and are concatenated to both the low-pass and
    high-pass deformation branches before patch embedding.
    """
    ...
    def __init__(
        ...
        super().__init__()
        branch_in_channels = 1 + context_channels
        patch_size = (time_patch, spatial_patch, spatial_patch)
        self.low_embed = PatchEmbed3D(
            in_channels=branch_in_channels,
            embed_dim=base_dim,
            patch_size=patch_size,
        )
        self.high_embed = PatchEmbed3D(
            in_channels=branch_in_channels,
            embed_dim=base_dim,
            patch_size=patch_size,
        )
        ...
        self.load_expand = PatchExpand3D(in_dim=base_dim * 4, out_dim=base_dim * 2, scale=merge_scale)
        self.sg_expand = PatchExpand3D(in_dim=base_dim * 4, out_dim=base_dim * 2, scale=merge_scale)
        self.load_fuse = nn.Conv3d(base_dim * 4, base_dim * 2, kernel_size=1)
        self.sg_fuse = nn.Conv3d(base_dim * 4, base_dim * 2, kernel_size=1)
        self.load_decoder = SwinStage3D(dim=base_dim * 2, depth=2, num_heads=num_heads, window_size=window_size)
        self.sg_decoder = SwinStage3D(dim=base_dim * 2, depth=2, num_heads=num_heads, window_size=window_size)
        self.load_up = nn.ConvTranspose3d(base_dim * 2, base_dim, kernel_size=patch_size, stride=patch_size)
        self.sg_up = nn.ConvTranspose3d(base_dim * 2, base_dim, kernel_size=patch_size, stride=patch_size)
        self.load_head = nn.Conv3d(base_dim, 1, kernel_size=1)
        self.sg_head = nn.Conv3d(base_dim, 1, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        d_low = F.avg_pool3d(d_raw, kernel_size=3, stride=1, padding=1)
        d_high = d_raw - d_low
        low = self.low_embed(torch.cat([d_low, context], dim=1))
        high = self.high_embed(torch.cat([d_high, context], dim=1))
        enc = self.enc(torch.cat([low, high], dim=1))
        bottleneck = self.bottleneck(self.merge(enc))

        load_dec = self.load_expand(bottleneck)
        load_dec = self.load_decoder(self.load_fuse(torch.cat([load_dec, enc], dim=1)))
        load = self.load_head(self.load_up(load_dec))[:, :, -1]

        sg_dec = self.sg_expand(bottleneck)
        sg_dec = self.sg_decoder(self.sg_fuse(torch.cat([sg_dec, enc], dim=1)))
        sg = self.sg_head(self.sg_up(sg_dec))[:, :, -1]
        return torch.cat([load, sg], dim=1)


class ContextConditionedResidualSwinUNet3D(nn.Module):
    """Generic residual Swin U-Net for multi-channel state correction.

    Channel 0 is the deformation history. Remaining channels are contextual
    priors or auxiliary inputs that the network can use to predict a residual
    ...
    def __init__(
        ...
        self.expand = PatchExpand3D(in_dim=base_dim * 4, out_dim=base_dim * 2, scale=merge_scale)
        self.fuse = nn.Conv3d(base_dim * 4, base_dim * 2, kernel_size=1)
        self.decoder = SwinStage3D(dim=base_dim * 2, depth=2, num_heads=num_heads, window_size=window_size)
        self.up = nn.ConvTranspose3d(base_dim * 2, base_dim, kernel_size=patch_size, stride=patch_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        dec = self.expand(bottleneck)
        dec = self.decoder(self.fuse(torch.cat([dec, enc], dim=1)))
        out = self.head(self.up(dec))[:, :, -1]
        return out
