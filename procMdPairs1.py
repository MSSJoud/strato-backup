...
def OnDelLineEvent(event):
    if event.key == 'delete' and selectedArtist:
        ...
        print('delete line:', [mdt.num2date(xd).strftime('%Y-%m-%d') for xd in xdata], ', remain pairs:', len(totalPairs))
...
aoi_bounds = get_aoi_bounds(setting.aoi_json)
...
fileList = [file for file in fileList if minLat <= float(file[37:44]) <= maxLat and minLon <= float(file[45:53]) <= maxLon]
...
for i in range(dates.shape[0]):
    ax.plot_date(dates[i], baseline[i], c='green')
